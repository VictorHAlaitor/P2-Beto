...
analisar_argumentos = { 
    'maior' : list(filter(lambda x: len(x)), map(lambda x : x > 0, resultado)), 
    'menor' : list(filter(lambda x: len(x)),map(lambda x : x < 0, resultado)),
    'media' : list(filter(lambda x: len(x)),map(lambda x: sum(x) / len(x), resultado)),
    'quantidade' : list(filter(lambda x: len(x)),map(lambda x : len(x), resultado ))
}
print("\n Teste ",analisar_argumentos)
