...
#umbers_input = input("Digite uma lista de números separados por vírgula: ")


#numbers_list = numbers_input.tri(',')
numbers_list = [ 1, 2, 3, 4, 5]
squared_numbers = list(map(lambda x:x **2, numbers_list))
print('\n Quadrados ', squared_numbers)

even_squares = list(filter(lambda x: x % 2 == 0, squared_numbers))

print("\n Quadrados pares ", even_squares)

total = reduce(lambda x, y : x + y, even_squares)

print("\n A soma dos cubos pares é: ", total)

cube_par = reduce(lambda x,y :  x > y or x < y,  even_squares)
print('\n Maior quadrado par ', cube_par)
